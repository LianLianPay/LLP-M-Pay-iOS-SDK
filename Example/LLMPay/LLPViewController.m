...
{
    [super viewDidLoad];
    self.version.text = [LLMPay getSDKVersion];
}

- (IBAction)PayAction:(UIButton *)sender {
    [[LLMPay sharedSdk] payApply:self.field.text complete:^(LLMPayResult result, NSDictionary *dic) {
        [self alertWithMsg:dic[@"ret_msg"]];
    }];
}

- (IBAction)signAction:(UIButton *)sender {
    [[LLMPay sharedSdk] signApply:self.field.text complete:^(LLMPayResult result, NSDictionary *dic) {
        [self alertWithMsg:dic[@"ret_msg"]];
    }];
}
